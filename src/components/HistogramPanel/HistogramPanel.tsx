...
export const HistogramPanel: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `histogram_${histogramType}_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <>
      ...
      <div className="statistics-section">
        ...
        {/* ビン数設定 */}
        <div className="control-group">
          <label>{t('histogram.binCount')}: {settings.binCount}</label>
          <input
            type="range"
            min="5"
            max="100"
        ...
        <div className="control-group">
          ...
          <button className="btn btn-secondary" onClick={handleExportCSV} disabled={!histogramData}>
            {t('histogram.export')}
          </button>
        </div>
        </div>
        ...
        <div className="statistics-section">
          ...
          <div className="statistics-grid">
            <div className="statistic-card">
              <div className="statistic-value">{formatValue(histogramData.statistics.mean, histogramData.type)}</div>
              <div className="statistic-label">{t('histogram.mean')}</div>
            </div>
            <div className="statistic-card">
              <div className="statistic-value">{formatValue(histogramData.statistics.median, histogramData.type)}</div>
              <div className="statistic-label">{t('histogram.median')}</div>
            </div>
            <div className="statistic-card">
              <div className="statistic-value">{formatValue(histogramData.statistics.std, histogramData.type)}</div>
              <div className="statistic-label">{t('histogram.std')}</div>
            </div>
            <div className="statistic-card">
              <div className="statistic-value">{formatValue(histogramData.statistics.min, histogramData.type)}</div>
              <div className="statistic-label">{t('histogram.min')}</div>
            </div>
            <div className="statistic-card">
              <div className="statistic-value">{formatValue(histogramData.statistics.max, histogramData.type)}</div>
              <div className="statistic-label">{t('histogram.max')}</div>
            </div>
            <div className="statistic-card">
              <div className="statistic-value">{histogramData.statistics.total.toLocaleString()}</div>
